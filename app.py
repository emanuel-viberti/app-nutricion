...
if 'db_platos' not in st.session_state:
    ...
    st.session_state.db_platos = {
        "dym": dym, "ayc": ayc, "trabajo": trabajo, "colacion": colaciones
    }
...
def elegir_plato(tipo, usados_hoy):
    lista_original = st.session_state.db_platos[tipo]
    # Filtrar para no repetir platos ya usados en la sesión si es posible
    opciones = [p for p in lista_original if p not in usados_hoy]
    if not opciones: # Si se acaban, resetear para esa categoría
        opciones = lista_original
...
with c2:
    peso_actual = st.number_input("Peso Actual (kg)", value=75.0)
    talla_cm = st.number_input("Talla (cm)", value=160.0)
with c3:
...
with cp1:
    if imc < 25:
        p_obj = st.number_input("Peso Ideal (Broca)", value=float(pi_broca))
    else:
        pic_wilkens = ((peso_actual - pi_broca) * 0.25) + pi_broca
        p_obj = st.number_input("Peso Ideal Corregido (Wilkens)", value=float(pic_wilkens))
...
colaciones_on = st.checkbox("Añadir 2 colaciones")
